...
namespace DofusMarket.Bot.Serialization;

internal class DofusBinaryReader : IDisposable
{
    private readonly BigEndianBinaryReader _reader;

    public DofusBinaryReader(Stream stream)
    {
        _reader = new BigEndianBinaryReader(stream, Encoding.UTF8);
    }
    ...
    /// <summary>Equivalent as readVarUhInt/readVarUhShort.</summary>
    public int Read7BitEncodedInt() => _reader.Read7BitEncodedInt();
    /// <summary>Equivalent as readVarUhLong.</summary>
